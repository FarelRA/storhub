...
func (h *StorHub) NewFUSE(project string, opts FUSEOptions) (*StorHubFS, error) {
	if err := validateProject(project); err != nil {
		return nil, err
	}
	defaults := DefaultFUSEOptions()
...
func (s *StorHubFS) Mount(mountPoint string) error {
...
func (s *StorHubFS) Wait() {
	if s.server != nil {
		s.server.Wait()
	}
}

func (s *StorHubFS) Unmount() error {
	if s.server == nil {
		return nil
	}
...
func (s *StorHubFS) pathForInode(inode uint64) string {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.inodePaths[inode]
}

func (s *StorHubFS) nodeForPathLocked(targetPath string) *storhubNode {
	if targetPath == "" {
		return s.root
	}
...
func (s *StorHubFS) rememberPath(inode uint64, targetPath string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.inodePaths[inode] = targetPath
}

func (s *StorHubFS) dropPath(inode uint64, targetPath string) string {
	s.mu.Lock()
	...
	s.mu.Unlock()
	meta, _, err := s.hub.loadRepoMetadataReadonly(context.Background(), s.project)
	s.mu.Lock()
	defer s.mu.Unlock()
	if err == nil {
...
func (s *StorHubFS) rebindHandlesAfterPathChange(inode uint64, oldPath, newPath string) {
	s.mu.RLock()
	handles := make([]*storhubHandle, 0, len(s.handles))
	for _, handle := range s.handles {
...
func (s *StorHubFS) remapPaths(oldPath, newPath string) {
	s.mu.Lock()
	for inode, current := range s.inodePaths {
		if isParentOrSame(oldPath, current) {
			s.inodePaths[inode] = remapPath(oldPath, newPath, current)
		}
	}
	handles := make([]*storhubHandle, 0, len(s.handles))
	for _, handle := range s.handles {
	...
	for _, handle := range handles {
		handle.mu.Lock()
		if isParentOrSame(oldPath, handle.path) {
			handle.path = remapPath(oldPath, newPath, handle.path)
		}
		handle.mu.Unlock()
	}
	s.mu.RLock()
	for _, writeState := range s.writeStates {
		writeState.mu.Lock()
		if isParentOrSame(oldPath, writeState.path) {
			writeState.path = remapPath(oldPath, newPath, writeState.path)
		}
		writeState.mu.Unlock()
	}
	s.mu.RUnlock()
...
func (s *StorHubFS) materializeHandlesForPath(ctx context.Context, inode uint64, targetPath string) error {
	s.mu.RLock()
	handles := make([]*storhubHandle, 0, len(s.handles))
	for _, handle := range s.handles {
		if handle.inode == inode {
...
func (s *StorHubFS) notifyEntryForPath(dirPath, name string) {
	s.mu.RLock()
...
func (s *StorHubFS) ensureNode(ctx context.Context, entry *EntryInfo) *storhubNode {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (n *storhubNode) stableAttr() gofusefs.StableAttr {
	...
	} else if n.kind == NodeKindSymlink {
...
func (s *StorHubFS) newHandle(ctx context.Context, inode uint64, flags uint32) (*storhubHandle, error) {
...
func (s *StorHubFS) acquireWriteState(ctx context.Context, inode uint64, targetPath string) (*inodeWriteState, error) {
	s.mu.Lock()
...
func (s *StorHubFS) releaseWriteState(state *inodeWriteState) {
	if state == nil {
		return
	}
	shouldClose := false
...
func (s *StorHubFS) hasOtherWriteHandle(inode uint64, excludeID uint64) bool {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (w *inodeWriteState) plannedRangesLocked() []byteRange {
	chunkSize := normalizedChunkSize(w.fs.hub.config.ChunkSize)
	if chunkSize <= 0 {
...
func (w *inodeWriteState) writeRangeToLocked(ctx context.Context, out *os.File, start, end int64) error {
	bufSize := normalizedChunkSize(w.fs.hub.config.ChunkSize)
	if bufSize <= 0 {
...
func (w *inodeWriteState) createRangeSnapshotLocked(ctx context.Context, ranges []byteRange) (string, error) {
	...
	buf := make([]byte, normalizedChunkSize(w.fs.hub.config.ChunkSize))
	if len(buf) == 0 {
...
func (h *storhubHandle) commit(ctx context.Context) syscall.Errno {
	if h.writeState != nil {
		...
		if len(h.writeState.dirtyRanges) == 0 {
			...
			h.fs.notifyEntryForPath(parentPath(targetPath), path.Base(targetPath))
			return 0
		}
		...
		if h.writeState.shouldChunkRewriteLocked(planned) {
			...
			repoMeta, _, err := h.fs.hub.loadRepoMetadataReadonly(ctx, h.fs.project)
			if err != nil {
				return errnoFromError(err)
			}
...
func (s *StorHubFS) setLock(inode, owner uint64, lk fuse.FileLock) syscall.Errno {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *StorHubFS) readCached(ctx context.Context, inode uint64, offset, length int64) ([]byte, error) {
	if length <= 0 {
		return []byte{}, nil
	}
...
func (s *StorHubFS) getPage(ctx context.Context, inode uint64, page int64) ([]byte, error) {
...
func (s *StorHubFS) evictInodeCache(inode uint64) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *StorHubFS) cleanupExpiredCache() {
	now := time.Now()
	s.mu.Lock()
	for key, elem := range s.pageCache {
		entry := elem.Value.(*pageCacheEntry)
...
func (s *StorHubFS) renameWithReplace(ctx context.Context, oldPath, newPath string, flags uint32) error {
	...
	_, err := s.hub.updateRepoMetadata(ctx, s.project, func(meta *RepoMetadata) error {
		if parent := parentPath(newPath); parent != "" && !meta.HasDirectory(parent) {
		...
		if file := meta.FindFile(oldPath); file != nil {
			...
			renamed.ChangedAt = s.hub.config.Now().UTC()
			meta.RemoveFile(oldPath)
			meta.UpsertFile(renamed, s.hub.config.Now().UTC())
			return nil
		}
		if !meta.HasDirectory(oldPath) {
			return fmt.Errorf("path not found: %s", oldPath)
		}
		if isParentOrSame(oldPath, newPath) {
		...
		for i := range meta.Directories {
			if isParentOrSame(oldPath, meta.Directories[i].Path) {
				meta.Directories[i].Path = remapPath(oldPath, newPath, meta.Directories[i].Path)
				meta.Directories[i].ModifiedAt = s.hub.config.Now().UTC()
				meta.Directories[i].ChangedAt = s.hub.config.Now().UTC()
			}
		}
		for i := range meta.Releases {
			for j := range meta.Releases[i].Files {
				if isParentOrSame(oldPath, meta.Releases[i].Files[j].Name) {
					meta.Releases[i].Files[j].Name = remapPath(oldPath, newPath, meta.Releases[i].Files[j].Name)
					meta.Releases[i].Files[j].ChangedAt = s.hub.config.Now().UTC()
				}
			}
		}
		meta.RecomputeStats()
...
func fillEntryOut(out *fuse.EntryOut, entry *EntryInfo, opts FUSEOptions) {
...
func fillAttr(attr *fuse.Attr, entry *EntryInfo) {
...
func errnoFromError(err error) syscall.Errno {
	...
	if errors.Is(err, ErrFileNotFound) || errors.Is(err, ErrProjectNotFound) {
		return syscall.ENOENT
	}
