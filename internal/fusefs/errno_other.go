...
// O_DSYNC-only opens there degrade to buffered durability. The degrade is
// undetectable per open (no bit to test), so it is logged once per mount
// in New instead of at each open.
