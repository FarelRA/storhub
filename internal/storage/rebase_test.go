...
// TestRebasePreservesBothWriters is the core Phase B contract: a rival
...
func TestRebaseLWWOnSamePath(t *testing.T) {
	...
	// Our later put wins (last-writer-wins per path).
	ops := []Op{
	...
	if got == nil || got.Size != 0 {
		t.Fatalf("expected our put to win (size 0), got %+v", got)
	}
