...
func (h *StorHub) PruneReq(ctx context.Context, project string, req PruneRequest) (*PruneResult, error) {
	...
	logging.Debug(h.projectLogger(project), "prune start", "scope", string(req.Scope), "keep", req.Keep, "dry_run", req.DryRun)
	...
	logging.Debug(h.projectLogger(project), "prune complete", "scope", string(req.Scope), "keep", req.Keep, "dry_run", req.DryRun, "deleted_objects", res.DeletedObjects, "deleted_releases", res.DeletedReleases, "deleted_assets", res.DeletedAssets, "history_compacted", res.HistoryCompacted, "elapsed", h.config.Now().UTC().Sub(started))
	return res, nil
}
...
func (h *StorHub) pruneAssets(ctx context.Context, project string, res *PruneResult, guard *pruneFenceGuard) error {
	...
	if res.DryRun {
		...
		logging.Debug(h.projectLogger(project), "prune assets complete", "scope", "assets", "dry_run", true, "count", len(releaseTasks)+len(assetTasks), "reclaimed", 0, "elapsed", h.config.Now().UTC().Sub(started))
		return nil
	}
	...
	if err := h.deletePurgePlan(ctx, project, releaseTasks, assetTasks, res); err != nil {
		logging.Error(h.projectLogger(project), "prune assets failed", "scope", "assets", "dry_run", false, "elapsed", h.config.Now().UTC().Sub(started), "err", err)
		return err
	}
	guard.drop()
	defer guard.rehold()
	...
	logging.Debug(h.projectLogger(project), "prune assets complete", "scope", "assets", "dry_run", false, "count", len(releaseTasks)+len(assetTasks), "reclaimed", res.DeletedReleases+res.DeletedAssets, "elapsed", h.config.Now().UTC().Sub(started))
	return nil
}
...
func (h *StorHub) pruneChunks(ctx context.Context, project string, res *PruneResult, guard *pruneFenceGuard) error {
	started := h.config.Now().UTC()
	logging.Debug(h.projectLogger(project), "prune chunks start", "scope", "chunks", "dry_run", res.DryRun)
	gc, err := h.ScanChunkGC(ctx, project)
	if err != nil {
		logging.Error(h.projectLogger(project), "prune chunks failed", "scope", "chunks", "dry_run", res.DryRun, "elapsed", h.config.Now().UTC().Sub(started), "err", err)
		return err
	}
	...
	if res.DryRun {
		logging.Debug(h.projectLogger(project), "prune chunks complete", "scope", "chunks", "dry_run", true, "scanned", res.ScannedChunks, "orphaned", res.OrphanChunks, "reclaimed", 0, "elapsed", h.config.Now().UTC().Sub(started))
		return nil
	}
	...
	if err != nil {
		logging.Error(h.projectLogger(project), "prune chunks failed", "scope", "chunks", "dry_run", false, "elapsed", h.config.Now().UTC().Sub(started), "err", err)
		return err
	}
	res.CollectedChunks = got.CollectedChunks
	res.CollectedBytes = got.CollectedBytes
	logging.Debug(h.projectLogger(project), "prune chunks complete", "scope", "chunks", "dry_run", false, "scanned", res.ScannedChunks, "orphaned", res.OrphanChunks, "reclaimed", res.CollectedChunks, "reclaimed_bytes", res.CollectedBytes, "elapsed", h.config.Now().UTC().Sub(started))
	return nil
}
...
func (h *StorHub) pruneObjects(ctx context.Context, project string, res *PruneResult, dryRun bool) error {
	started := h.config.Now().UTC()
	logging.Debug(h.projectLogger(project), "prune objects start", "scope", "objects", "dry_run", dryRun)
	...
	if dryRun || len(orphans) == 0 {
		logging.Debug(h.projectLogger(project), "prune objects complete", "scope", "objects", "dry_run", dryRun, "count", len(orphans), "reclaimed", 0, "elapsed", h.config.Now().UTC().Sub(started))
		return nil
	}
	...
	if err := h.deleteRepoObjects(ctx, project, orphans, cache); err != nil {
		logging.Error(h.projectLogger(project), "prune objects failed", "scope", "objects", "dry_run", dryRun, "count", len(orphans), "elapsed", h.config.Now().UTC().Sub(started), "err", err)
		return err
	}
	logging.Debug(h.projectLogger(project), "prune objects complete", "scope", "objects", "dry_run", dryRun, "count", len(orphans), "reclaimed", len(orphans), "elapsed", h.config.Now().UTC().Sub(started))
	return nil
}
...
func (h *StorHub) pruneHistory(ctx context.Context, project string, keep int, res *PruneResult, dryRun bool) error {
	started := h.config.Now().UTC()
	logging.Debug(h.projectLogger(project), "prune history start", "scope", "history", "keep", keep, "dry_run", dryRun)
	if keep > 1 {
	...
	if repo == nil {
		...
		logging.Debug(h.projectLogger(project), "prune history complete", "scope", "history", "keep", keep, "dry_run", dryRun, "count", 0, "reclaimed", 0, "elapsed", h.config.Now().UTC().Sub(started))
		return nil
	}
	revs, err := repo.listFileCommits(ctx, indexFilePath)
	if err != nil {
		logging.Error(h.projectLogger(project), "prune history failed", "scope", "history", "keep", keep, "dry_run", dryRun, "elapsed", h.config.Now().UTC().Sub(started), "err", err)
		return err
	}
	if len(revs) <= keep {
		res.Notes = append(res.Notes, fmt.Sprintf("history: %d manifest commits, at or below keep=%d; nothing to compact", len(revs), keep))
		logging.Debug(h.projectLogger(project), "prune history complete", "scope", "history", "keep", keep, "dry_run", dryRun, "count", len(revs), "reclaimed", 0, "elapsed", h.config.Now().UTC().Sub(started))
		return nil
	}
	if dryRun {
		...
		logging.Debug(h.projectLogger(project), "prune history complete", "scope", "history", "keep", keep, "dry_run", true, "count", len(revs), "reclaimed", 0, "elapsed", h.config.Now().UTC().Sub(started))
		return nil
	}
	if err := h.ensureOwner(ctx); err != nil {
		logging.Error(h.projectLogger(project), "prune history failed", "scope", "history", "keep", keep, "dry_run", dryRun, "elapsed", h.config.Now().UTC().Sub(started), "err", err)
	...
	logging.Debug(h.projectLogger(project), "prune history complete", "scope", "history", "keep", keep, "dry_run", dryRun, "count", len(revs), "reclaimed", 1, "elapsed", h.config.Now().UTC().Sub(started))
	return nil
}
