...
func TestPurgeObjectsRemovesOrphans(t *testing.T) {
	t.Parallel()
	...
	dry, err := hub.Purge(ctx, "pruneobj", PurgeObjects, 0, true)
	if err != nil {
	...
	res, err := hub.Purge(ctx, "pruneobj", PurgeObjects, 0, false)
	if err != nil {
...
func TestPurgeObjectsAbortsWhenRevisionUnreadable(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	backend := newMockGitHub(t)
	hub := backend.newClient(t, smallTransferTestConfig())
	...
	if _, err := hub.Purge(ctx, "c1sab", PurgeObjects, 0, false); err == nil {
	...
	res, err := hub.Purge(ctx, "c1sab", PurgeObjects, 0, false)
...
func TestPurgeObjectsRefusesTruncatedListing(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	backend := newMockGitHub(t)
	hub := backend.newClient(t, smallTransferTestConfig())
	...
	if _, err := hub.Purge(ctx, "cap", PurgeObjects, 0, false); err == nil {
...
func TestPurgePartialDeleteDropsCachedShas(t *testing.T) {
	...
	if _, err := hub.Purge(ctx, "m1", PurgeObjects, 0, false); err == nil {
	...
	res, err := hub.Purge(ctx, "m1", PurgeObjects, 0, false)
	if err != nil {
...
func TestPurgeObjectsLegacyIsNoop(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	backend := newMockGitHub(t)
	hub := backend.newClient(t, smallTransferTestConfig())
	...
	res, err := hub.Purge(ctx, "v1prune", PurgeObjects, 0, false)
	if err != nil {
...
func TestPurgeObjectsUninitializedIsNoop(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	backend := newMockGitHub(t)
	hub := backend.newClient(t, smallTransferTestConfig())
	...
	res, err := hub.Purge(ctx, "freshprune", PurgeObjects, 0, false)
	if err != nil {
...
func TestPurgeHistoryRESTRefusesHonestly(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	backend := newMockGitHub(t)
	hub := backend.newClient(t, smallTransferTestConfig())
	seedMeta(t, hub, "resthist", "docs", "a.txt", 1)
	res, err := hub.Purge(ctx, "resthist", PurgeHistory, 1, false)
	if err != nil {
...
func TestPurgeHistoryRejectsKeepAboveOne(t *testing.T) {
	t.Parallel()
	if testing.Short() {
	...
	if _, err := hub.Purge(ctx, "restkeep", PurgeHistory, 50, false); err == nil ||
	...
	if _, err := gith.Purge(ctx, "demo", PurgeHistory, 50, false); err == nil ||
		!strings.Contains(err.Error(), "keep=50") {
	...
	dry, err := gith.Purge(ctx, "demo", PurgeHistory, 1, true)
	if err != nil {
	...
	res, err := gith.Purge(ctx, "demo", PurgeHistory, 0, false)
	if err != nil {
...
func TestPurgeHistoryGitCompactsAndPreservesObjects(t *testing.T) {
	t.Parallel()
	if testing.Short() {
		t.Skip("git-backend test: heavy go-git fixture, skipped in short mode")
	}
	...
	res, err := hub.Purge(ctx, "demo", PurgeHistory, 1, false)
	if err != nil {
...
// PurgeAssets must purge the untracked release exactly: the untracked release with an
// asset is reclaimed, the tracked release AND its referenced asset survive,
// and the reported counts are the truth, not a sum.
func TestPurgeAssetsWiresPurge(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	backend := newMockGitHub(t)
	hub := backend.newClient(t, smallTransferTestConfig())
	...
	// An empty untracked release is skipped by the assets purge (curated
	...
	res, err := hub.Purge(ctx, "pa", PurgeAssets, 0, false)
	if err != nil {
...
// PurgeAll = history (honestly refused on REST) + objects + assets, with
// exact per-stage counts.
func TestPurgeAllReclaimsObjectsAndAssets(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	backend := newMockGitHub(t)
	hub := backend.newClient(t, smallTransferTestConfig())
	...
	res, err := hub.Purge(ctx, "all", PurgeAll, 1, false)
	if err != nil {
...
// PurgeContext is the string-scope adapter for the REST layer: valid scopes
// map to the typed scopes and unknown scopes are rejected, not silently
// treated as no-ops.
func TestPurgeContextScopeAdapter(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	backend := newMockGitHub(t)
	hub := backend.newClient(t, smallTransferTestConfig())
	...
	for _, s := range []string{"objects", "assets", "history", "all"} {
		res, err := hub.PurgeContext(ctx, "scope", s, 1, true)
		if err != nil {
			t.Fatalf("scope %q: %v", s, err)
		}
		if res.Scope != PurgeScope(s) {
			t.Fatalf("scope %q echoed as %q", s, res.Scope)
		}
	}
	if _, err := hub.PurgeContext(ctx, "scope", "garbage", 1, false); err == nil ||
		!strings.Contains(err.Error(), "unknown purge scope") {
		t.Fatalf("expected unknown-scope error, got %v", err)
	}
	// The context-free CLI wrapper reaches the same adapter.
	if _, err := hub.PurgeProject("scope", "bogus", 1, false); err == nil ||
		!strings.Contains(err.Error(), "unknown purge scope") {
		t.Fatalf("PurgeProject must surface the unknown-scope error, got %v", err)
	}
}

// Every purge scope must refuse a project with uncommitted metadata changes:
...
func TestPurgeRefusesDirtyProject(t *testing.T) {
	...
	for _, scope := range []PurgeScope{PurgeObjects, PurgeAssets, PurgeHistory, PurgeAll} {
		if _, err := hub.Purge(ctx, "dirty", scope, 1, false); err == nil ||
			!strings.Contains(err.Error(), "purge refused") {
			t.Fatalf("scope %s: expected purge refusal for a dirty project, got %v", scope, err)
		}
	}
	backend.intercept.Store(func(http.ResponseWriter, *http.Request) bool { return false })
}
...
func TestPurgeEnumeratesWideShardWithTwoListDirs(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	backend := newMockGitHub(t)
	hub := backend.newClient(t, smallTransferTestConfig())
