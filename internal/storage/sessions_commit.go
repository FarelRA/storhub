...
// state must publish to, in publish order: the single resolved path for
// opened-with-path handles, the pending set for linked handles. It is the
// one resolver behind both commit lanes; per-name publishing goes through
// the shared create and image helpers below. Callers must hold s.mu.
...
func (h *StorHub) commitSessionLocked(ctx context.Context, sh *sessionHubState, s *openSession) (string, error) {
	...
	if s.path == "" {
		return "", fmt.Errorf("commit session %s: %w", s.handleID, ErrSessionUnlinked)
	}
	...
	if commitPath == "" {
		return "", fmt.Errorf("commit session %s: %w", s.handleID, ErrSessionPathGone)
	}
	if commitPath != s.path {
		logging.Warn(h.projectLogger(s.project), "session-commit followed rename", "handle", shortSHA(s.handleID), "project", s.project, "path", s.path, "commit_path", commitPath)
	}
	...
	if !s.applied {
		...
		switch {
		case s.created:
			_, err = h.uploadSessionImageLocked(commitCtx, s, commitPath)
		case s.fullImage:
			var staged *os.File
			...
			if err != nil {
				err = fmt.Errorf("commit session %s: %w", s.handleID, err)
			}
		...
		default:
			...
			if err == nil {
				if len(edits) == 0 {
					return "", fmt.Errorf("commit session %s: staged state with no dirty ranges", s.handleID)
				}
	...
	if !s.applied {
		if s.created {
			for _, name := range pending {
				...
				if _, err := h.uploadSessionImageLocked(commitCtx, s, name); err != nil {
					return "", err
				}
				if s.published == nil {
...
// uploadSessionImageLocked uploads the full staged image of a handle that
// is creating its target. The single-path commit and the linked fan-out
// share it so both lanes spell creation one way. Caller holds s.mu and
// the hub commit mutex.
func (h *StorHub) uploadSessionImageLocked(ctx context.Context, s *openSession, name string) (*FileMeta, error) {
	return h.UploadFileContext(ctx, s.project, name, s.tmpName)
}
...
func (h *StorHub) publishSessionImageLocked(ctx context.Context, s *openSession, name string, live *RepoMetadata) error {
	if live.FindFile(name) != nil {
		staged, err := os.Open(s.tmpName)
		if err != nil {
			return fmt.Errorf("commit session %s: %w", s.handleID, err)
		}
		defer func() { _ = staged.Close() }()
...
func (h *StorHub) repinSessionLocked(ctx context.Context, s *openSession, commitPath string) error {
	...
	if entry == nil {
		return fmt.Errorf("repin session %s: %w: %s", s.handleID, shfs.ErrNotFound, commitPath)
	}
	pinned := entry.Clone()
	...
	if s.tmp != nil {
		if err := s.tmp.Truncate(0); err != nil {
			return fmt.Errorf("repin session %s: %w", s.handleID, err)
		}
	}
	return nil
}
...
// discardSession forgets one handle without committing anything. It is the
// shared primitive behind DiscardSession and the no-commit branches of
// CloseSession, so close-versus-discard stays visible at the call site.
// The caller holds s.mu and must not hold sh.mu (see destroySession for
// the lock dance).
func (sh *sessionHubState) discardSession(handleID string, s *openSession) error {
...
func (h *StorHub) SyncSession(ctx context.Context, handleID string) (err error) {
	...
	if aerr := s.authorize(ctx); aerr != nil {
		logging.Error(h.projectLogger(s.project), "session auth failed", "handle", shortSHA(s.handleID), "project", s.project, "path", s.path, "op", "sync", "err", aerr)
		return aerr
	}
	started := h.config.Now().UTC()
	logging.Debug(h.projectLogger(s.project), "session-sync start", "handle", shortSHA(s.handleID), "project", s.project, "path", s.path)
	defer func() {
		elapsed := h.config.Now().UTC().Sub(started)
		if err != nil {
			logging.Error(h.projectLogger(s.project), "session-sync failed", "handle", shortSHA(s.handleID), "project", s.project, "path", s.path, "elapsed", elapsed, "err", err)
			return
		}
		logging.Debug(h.projectLogger(s.project), "session-sync complete", "handle", shortSHA(s.handleID), "project", s.project, "path", s.path, "elapsed", elapsed)
	}()
	if !s.dirty {
		s.lastUse = sh.now()
		return nil
	...
	if cerr != nil {
		if errors.Is(cerr, ErrSessionPathGone) {
			...
			// See ErrSessionPathGone: sync retains, close discards.
			logging.Warn(h.projectLogger(s.project), "session-sync on unlinked path; retaining staged state", "handle", shortSHA(s.handleID), "project", s.project, "path", s.path)
			s.lastUse = sh.now()
			return nil
		}
		return cerr
...
func (h *StorHub) CloseSession(ctx context.Context, handleID string) (err error) {
	...
	// Both no-commit branches below discard: close without staged state,
	// and close after the pinned path went away.
	discard := func() error {
		return sh.discardSession(handleID, s)
	}
	if s.path == "" || !s.dirty {
		return discard()
	}
	if _, err := h.commitSessionLocked(ctx, sh, s); err != nil {
		if errors.Is(err, ErrSessionPathGone) {
			...
			// discards the staged state with success. See
			// ErrSessionPathGone: sync retains, close discards.
			dproject, dpath, dhandle := s.project, s.path, shortSHA(s.handleID)
...
// DiscardSession forgets one handle without committing anything: staged
// bytes are dropped, linked names publish nothing, and the handle answers
// stale afterwards. Discarding a clean handle is a no-op success.
// Table lock covers lookup and final destroy only.
func (h *StorHub) DiscardSession(ctx context.Context, handleID string) (err error) {
	sh := h.sessionHub()
	sh.mu.Lock()
	s, lerr := sh.getLiveLocked(handleID, sh.now())
	if lerr != nil {
		sh.mu.Unlock()
		logging.Error(h.logger, "session-discard lookup failed", "handle", shortSHA(handleID), "op", "discard", "err", lerr)
		return lerr
	}
	sh.mu.Unlock()
	if aerr := s.authorize(ctx); aerr != nil {
		aproject, apath, ahandle := s.project, s.path, shortSHA(s.handleID)
		s.mu.Unlock()
		logging.Error(h.projectLogger(aproject), "session auth failed", "handle", ahandle, "project", aproject, "path", apath, "op", "discard", "err", aerr)
		return aerr
	}
	started := h.config.Now().UTC()
	sproject, spath, shandle := s.project, s.path, shortSHA(s.handleID)
	logging.Debug(h.projectLogger(sproject), "session-discard start", "handle", shandle, "project", sproject, "path", spath)
	defer func() {
		elapsed := h.config.Now().UTC().Sub(started)
		if err != nil {
			logging.Error(h.projectLogger(sproject), "session-discard failed", "handle", shandle, "project", sproject, "path", spath, "elapsed", elapsed, "err", err)
			return
		}
		logging.Debug(h.projectLogger(sproject), "session-discard complete", "handle", shandle, "project", sproject, "path", spath, "elapsed", elapsed)
	}()
	return sh.discardSession(handleID, s)
}
