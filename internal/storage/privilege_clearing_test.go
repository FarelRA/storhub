...
func TestUnprivilegedPutClearsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, userCtx, target := setupPrivProject(t, "project-put-clear", "docs", "f.txt", []byte("hello world"), 0o6777)
...
func TestAdminPutKeepsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, _, target := setupPrivProject(t, "project-put-keep", "docs", "f.txt", []byte("hello world"), 0o6777)
	input := writeTempFile(t, t.TempDir(), "new.txt", []byte("replaced!!"))
...
func TestUnprivilegedPatchClearsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, userCtx, target := setupPrivProject(t, "project-patch-clear", "docs", "f.txt", []byte("hello world"), 0o6777)
...
func TestAdminPatchKeepsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, _, target := setupPrivProject(t, "project-patch-keep", "docs", "f.txt", []byte("hello world"), 0o6777)
...
func TestUnprivilegedPatchRangesClearsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, userCtx, target := setupPrivProject(t, "project-ranges-clear", "docs", "f.txt", []byte("hello world"), 0o6777)
...
func TestUnprivilegedReplaceReaderClearsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, userCtx, target := setupPrivProject(t, "project-replace-reader", "docs", "f.txt", []byte("hello world"), 0o6777)
...
func TestUnprivilegedRewriteRangesClearsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, userCtx, target := setupPrivProject(t, "project-rewrite-clear", "docs", "f.txt", []byte("hello world"), 0o6777)
	snapshot := writeTempFile(t, t.TempDir(), "snap.txt", []byte("HELLO world"))
	repo, _, err := hub.LoadRepoMetadataContext(userCtx, "project-rewrite-clear")
	if err != nil {
...
func TestUnprivilegedTruncateShrinkClearsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, userCtx, target := setupPrivProject(t, "project-trunc-shrink", "docs", "f.txt", []byte("hello world"), 0o6666)
...
func TestUnprivilegedTruncateNoopClearsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, userCtx, target := setupPrivProject(t, "project-trunc-noop", "docs", "f.txt", []byte("hello world"), 0o6666)
	info, err := hub.StatPathContext(adminCtx, "project-trunc-noop", target)
	if err != nil {
		t.Fatalf("stat: %v", err)
	}
...
func TestAdminTruncateNoopKeepsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, _, target := setupPrivProject(t, "project-trunc-noop-keep", "docs", "f.txt", []byte("hello world"), 0o6666)
	info, err := hub.StatPathContext(adminCtx, "project-trunc-noop-keep", target)
	if err != nil {
		t.Fatalf("stat: %v", err)
	}
...
func TestUnprivilegedWriteAtClearsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, userCtx, target := setupPrivProject(t, "project-writeat-clear", "docs", "f.txt", []byte("hello world"), 0o6666)
...
func TestUnprivilegedAppendClearsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, userCtx, target := setupPrivProject(t, "project-append-clear", "docs", "f.txt", []byte("hello world"), 0o6666)
...
func TestAdminChownKeepsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, _, target := setupPrivProject(t, "project-chown-keep", "docs", "f.txt", []byte("hello world"), 0o6777)
...
func TestAdminChownKeepsDirPrivilegeBits(t *testing.T) {
	...
	hub := backend.newClient(t, smallTransferTestConfig())
	adminCtx := privAdminCtx()
	project := "project-chown-dir-keep"
	if err := hub.MkdirContext(adminCtx, project, "docs"); err != nil {
		t.Fatalf("mkdir docs: %v", err)
	}
...
func TestTruncateGrowClearsPrivilegeBits(t *testing.T) {
	t.Parallel()
	hub, adminCtx, userCtx, target := setupPrivProject(t, "project-trunc-grow", "docs", "f.txt", []byte("hi"), 0o6666)
