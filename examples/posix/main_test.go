...
func TestMainRequiresToken(t *testing.T) {
	helper := os.Args[0]
	cmd := exec.Command(helper, "-test.run=TestPOSIXHelperProcess", "--", "main")
	cmd.Env = append(os.Environ(), "GO_WANT_POSIX_HELPER=1")
	out, err := cmd.CombinedOutput()
	if err == nil {
		t.Fatal("expected posix example to fail without token")
	}
